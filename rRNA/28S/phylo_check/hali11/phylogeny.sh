...
#	do cd-hit -i $i -o $(basename $i .fa).cdhit -c 0.90
#done

#alignment:
for i in *.cdhit;
      do mafft --anysymbol --reorder --auto --adjustdirection  $i > $(basename $i .ren).aln ;
done
...
#RAxML tree building:
for i in *.aln ;
        do raxml-ng --all --bs-trees 20 --msa $i --model GTR+G ;
done
