#!/bin/bash


#SBATCH --time=2-02:30:00 # ask that the job be allowed to run for 2 days, 2 hours, 30 minutes.
#SBATCH --nodes=1   # number of nodes
#SBATCH --ntasks-per-node=16   
#SBATCH --mem=8G # Maximum amount of memory this job will be given
#SBATCH --job-name="phytree"
#SBATCH --open-mode=append
#SBATCH --output=phylo.out
#SBATCH --mail-user=makhaki@iastate.edu
#SBATCH --mail-type=FAIL,END


# LOAD MODULES, INSERT CODE, AND RUN YOUR PROGRAMS HERE
module load mafft/7.481-le52rau
module load gblocks
module load dafoam/1.0
module load cdhit/4.8.1-7sf2emj
module load raxml-ng/1.0.2-py39-openmpi4-srxrtsm


#rename sequences:

#for i in *.fasta;
#do sed 's/>[A-Za-z0-9]\+|\([A-Za-z0-9_]\+\)|.\+ OS=\([A-Za-z\-]\+\) \([a-zA-Z0-9\.]\+\) .\+/>\1_\2_\3/' $i > $(basename $i .fasta).ren ;
#done

#cd-hit 
for i in *.fa
        do cd-hit -i $i -o $(basename $i .fa).cdhit -c 0.80
done

#alignment:
#for i in *.cdhit;
#        do mafft --reorder --auto $i > $(basename $i .ren).aln ;
#done

#filter alignments:
#for i in *.aln;
#        do Gblocks $i -b5=a -p=n  ;# -b3=15 -b4=5  -b2=0.5 ;
#done

#RAxML tree building:
#echo $(which raxmlHPC-PTHREADS)
#for i in *.aln-gb ;
#        do raxml-ng --all --msa $i --model WAG ;
#done
